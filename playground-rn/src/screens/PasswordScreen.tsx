...
import { zipWithPassword, isPasswordProtected, EncryptionMethods } from 'react-native-zip-archive';
import { useUnzip } from '../hooks/useUnzip';
import { ResultCard } from '../components/ResultCard';
import { CodePreview } from '../components/CodePreview';
import { ensureDir, listFiles } from '../utils/fileSystem';

const ENCRYPTIONS: EncryptionMethods[] = [
  EncryptionMethods.STANDARD,
  EncryptionMethods.AES_128,
...
export default function PasswordScreen() {
  ...
  const [encryption, setEncryption] = useState<EncryptionMethods>(EncryptionMethods.AES_256);
  ...
  const createPasswordZip = async () => {
    ...
    setExtractedFiles([]);
    const dir = RNFS.DocumentDirectoryPath + '/password-demo/';
    await ensureDir(dir);
    await RNFS.writeFile(dir + 'private.txt', 'This is secret content', 'utf8');
    const target = RNFS.DocumentDirectoryPath + '/password-demo.zip';
    try {
  ...
  const handleUnzip = async () => {
    ...
    reset();
    setExtractedFiles([]);
    const out = RNFS.DocumentDirectoryPath + '/password-unzipped/' + Date.now() + '/';
    await ensureDir(out);
    try {
